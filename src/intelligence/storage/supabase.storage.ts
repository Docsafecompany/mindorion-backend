...
export async function storeProposalAnalysis(
  ...
  try {
      ...
      if (output.risk_categories && output.risk_categories.length > 0) {
              ...
              const { error: signalsError } = await supabase.from('proposal_signals').insert(signals as any);
      ...
      if (output.proposal_dimensions && output.proposal_dimensions.length > 0) {
              ...
              const { error: dimsError } = await supabase.from('proposal_dimension_scores').insert(dims as any);
      ...
      if (output.strength_signals && output.strength_signals.length > 0) {
              ...
              const { error: strengthsError } = await supabase.from('proposal_strength_signals').insert(strengths as any);
