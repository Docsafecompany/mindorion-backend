...
router.post('/', upload.single('file'), async (req: Request, res: Response) => {
  try {
    ...
    // Run all detectors
    const [metadata, comments, trackChanges, hiddenContent, sensitiveData, hygiene,
      speakerNotes, embeddedObjects, macros, formulas, docFields, brokenLinks] = await Promise.all([
    ...
    const detections: any = {
      ...
      embeddedExcel: embeddedObjects.filter((o: any) => o.type === 'excel'),
      aiDetection,
    ...
    const { riskScore, riskLevel, totalIssues, criticalIssues, recommendations } = calculateRiskScore(detections);
    
    // For proposal mode, run business intelligence
    let proposalIntelligence = null;
    if (analysisMode === 'proposal' && !fast && parsedDoc.fullText) {
      proposalIntelligence = await mindorionIntelligenceService.runProposalIntelligence(
        parsedDoc.fullText, documentId, req.body.user_id, req.body.org_id
      );
    }
    ...
    const result = {
      ...
      detections,
      summary: { totalIssues, criticalIssues, riskScore, riskLevel, recommendations },
